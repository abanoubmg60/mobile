...
class _BoardState extends State<Chessboard> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final piecesPainter = PiecesPainter(
      ...
      premoveQueueNotifier: _controller.premoveQueueNotifier,
    ...
    final ghostPiecesPainter = GhostPiecesPainter(
      premoveQueueNotifier: _controller.premoveQueueNotifier,
      piecesNotifier: _controller.piecesNotifier,
      squareSize: widget.squareSize,
      orientation: widget.orientation,
      ...
      child: SizedBox.square(
        ...
        child: Stack(
          ...
          children: [
            ...
            CustomPaint(size: Size.square(widget.size), painter: ghostPiecesPainter),
            CustomPaint(
              size: Size.square(widget.size),
  ...
  void initState() {
    ...
    _controller.premoveQueueNotifier.addListener(_onPremoveChange);
  ...
  void dispose() {
    ...
    _controller.premoveQueueNotifier.removeListener(_onPremoveChange);
  ...
  void didUpdateWidget(Chessboard oldBoard) {
    ...
    if (oldBoard.controller != widget.controller) {
      ...
      oldBoard.controller.premoveQueueNotifier.removeListener(_onPremoveChange);
  ...
  void _syncHighlightNotifier() {
    ...
    final isOpponentTurn = game.playerSide.name == game.sideToMove.opposite.name;
    final premoveHighlight = premove != null && isOpponentTurn ? premove : null;
    final premoveQueue = isOpponentTurn ? _controller.premoveQueue : const <Move>[];
  ...
  void _onPointerDown(PointerDownEvent details) {
    ...
    final activePieces =
        widget.settings.multiplePremoves && _controller.premoveQueue.isNotEmpty
            ? _controller.virtualPieces
            : pieces;
    final Piece? piece = activePieces[square];
    ...
    // - cancel premove
    // - unselect piece
    else if (_controller.premoveQueue.isNotEmpty || _controller.premove != null) {
      _controller.clearPremoves();
      _setSelection(null);
    }

    // there is a premove set from the touched square:
    // - cancel the premoves on the next tap up event
    if (_controller.premoveQueue.any(
      (m) => switch (m) {
        NormalMove(:final from) => from == square,
        DropMove(:final to) => to == square,
      },
    )) {
  ...
  void _onPointerUp(PointerUpEvent details) {
    ...
    if (_dragAvatar != null) {
      ...
      if (square != null) {
        if (square != selected) {
          ...
          if (!couldMove && (_controller.premoveQueue.isNotEmpty || _controller.premove != null)) {
            _controller.clearPremoves();
          }
        } else {
    ...
    // cancel premove if the user taps on an origin or destination square of the premove
  ...
  void _onDragStart(PointerEvent origin) {
    ...
    final activePieces =
        widget.settings.multiplePremoves && _controller.premoveQueue.isNotEmpty
            ? _controller.virtualPieces
            : pieces;
    final piece = square != null ? activePieces[square] : null;
    final feedbackSize =
  ...
  bool _canPremoveTo(Square orig, Square dest, [Pieces? pieceSet]) {
    final activePieces =
        pieceSet ??
        (widget.settings.multiplePremoves && _controller.premoveQueue.isNotEmpty
            ? _controller.virtualPieces
  ...
  bool _tryMoveOrPremoveTo(Square square, {bool drop = false}) {
    final activePieces =
        widget.settings.multiplePremoves && _controller.premoveQueue.isNotEmpty
            ? _controller.virtualPieces
            : pieces;
    final selectedPiece = selected != null ? activePieces[selected] : null;
    if (selectedPiece != null && _canMoveTo(selected!, square)) {
      ...
      return true;
    } else if (_isPremovable(selectedPiece) && _canPremoveTo(selected!, square, activePieces)) {
      ...
      if (widget.settings.multiplePremoves) {
        _controller.addPremove(premove);
      } else {
