...
class ChessboardController extends ChangeNotifier {
  ...
  ChessboardController({required GameData game}) {
    ...
    _premoveQueueNotifier = ValueNotifier([]);
  }
  ...
  late final ValueNotifier<List<Move>> _premoveQueueNotifier;
  ...
  /// The list of currently queued premoves.
  List<Move> get premoveQueue => _premoveQueueNotifier.value;
  ...
  /// A notifier that fires whenever the premove queue changes.
  ValueNotifier<List<Move>> get premoveQueueNotifier => _premoveQueueNotifier;

  /// Returns the board pieces after applying all queued premoves in [premoveQueue].
  Pieces get virtualPieces {
    if (premoveQueue.isEmpty) return pieces;
    final map = Map<Square, Piece>.from(pieces);
    for (final move in premoveQueue) {
      switch (move) {
        case NormalMove(:final from, :final to, :final promotion):
          final p = map.remove(from);
          if (p != null) {
            final newPiece = promotion != null ? Piece(color: p.color, role: promotion) : p;
            map[to] = newPiece;
            if (p.role == Role.king) {
              final rank = from.rank;
              if (from.file == File.e && to.file == File.g) {
                final rook = map.remove(Square.fromCoords(File.h, rank));
                if (rook != null) map[Square.fromCoords(File.f, rank)] = rook;
              } else if (from.file == File.e && to.file == File.c) {
                final rook = map.remove(Square.fromCoords(File.a, rank));
                if (rook != null) map[Square.fromCoords(File.d, rank)] = rook;
              }
            }
          }
        case DropMove(:final role, :final to):
          final side = game.playerSide == PlayerSide.white ? Side.white : Side.black;
          map[to] = Piece(color: side, role: role);
      }
    }
    return map;
  }
  ...
  ///
  /// Assign a non-null [Move] to register a single premove, or `null` to clear it.
  ...
  set premove(Move? move) {
    if (move == null) {
      ...
      _premoveQueueNotifier.value = [move];
      _premoveNotifier.value = move;
    }
  }

  /// Appends a premove to the queue.
  void addPremove(Move move) {
    final newQueue = [..._premoveQueueNotifier.value, move];
    _premoveQueueNotifier.value = newQueue;
    _premoveNotifier.value = newQueue.firstOrNull;
  }

  /// Clears all queued premoves.
  void clearPremoves() {
    if (_premoveQueueNotifier.value.isNotEmpty) {
      _premoveQueueNotifier.value = const [];
    }
    if (_premoveNotifier.value != null) {
      _premoveNotifier.value = null;
    }
  }

  /// Removes and returns the first premove in the queue, or `null` if empty.
  Move? removeFirstPremove() {
    final current = _premoveQueueNotifier.value;
    if (current.isEmpty) return null;
    final first = current.first;
    final remaining = current.sublist(1);
    _premoveQueueNotifier.value = remaining;
    _premoveNotifier.value = remaining.firstOrNull;
    return first;
  }
