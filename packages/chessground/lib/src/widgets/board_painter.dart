...
class BoardHighlightNotifier extends ChangeNotifier {
  ...
  /// The queued premoves, if any.
  List<Move> premoveQueue = const [];

  /// The square of the king in check, if any.
  Square? checkSquare;

  /// Returns whether [square] is part of any active premove.
  bool isPremoveSquare(Square square) {
    if (premoveQueue.isNotEmpty) {
      return premoveQueue.any((m) => m.hasSquare(square));
    }
    return premove?.hasSquare(square) ?? false;
  }
  ...
  void update({
    ...
    this.premoveQueue = premoveQueue;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showLastMove && lastMove != null) {
      for (final square in lastMove.squares) {
        if (!interactionNotifier.isPremoveSquare(square)) {
          _drawHighlight(canvas, _squareRect(square, squareSize, orientation), lastMoveDetails);
        }
      }
    }

    final premoveQueue = interactionNotifier.premoveQueue;
    if (premoveQueue.isNotEmpty) {
      final paint = Paint()..color = premoveColor;
      for (final move in premoveQueue) {
        for (final square in move.squares) {
      ...
      if (premoveQueue.length > 1) {
        ...
        for (int i = 0; i < premoveQueue.length; i++) {
          final move = premoveQueue[i];
          final destSquare = switch (move) {
            NormalMove(:final to) => to,
            DropMove(:final to) => to,
          };
          final rect = _squareRect(destSquare, squareSize, orientation);
          final badgeCenter = Offset(rect.right - 9.0, rect.top + 9.0);
...
class PiecesPainter extends CustomPainter {
    ...
    required this.imagesLoaded,
       ...
       super(
         repaint: Listenable.merge([
           ...
           if (premoveQueueNotifier != null) premoveQueueNotifier,
           gameNotifier,
  ...
  /// The queued premoves, used to dim origin pieces.
  final ValueNotifier<List<Move>>? premoveQueueNotifier;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final premoveOrigins = <Square>{
      for (final m in premoveQueueNotifier?.value ?? const <Move>[])
        if (m is NormalMove) m.from,
    };
    ...
    for (final entry in pieces.entries) {
      ...
      final isPremoveOrigin = premoveOrigins.contains(square);
      final paint =
          Paint()
...
/// Paints ghost pieces (semi-transparent) on the destination squares of queued premoves.
class GhostPiecesPainter extends CustomPainter {
  GhostPiecesPainter({
    required this.premoveQueueNotifier,
    required this.piecesNotifier,
    required this.squareSize,
    required this.orientation,
    required this.pieceAssets,
    ...
    required this.gameNotifier,
    required this.imagesLoaded,
  }) : super(repaint: premoveQueueNotifier);
  ...
  /// The ghost pieces currently positioned at their premove destinations.
  Map<Square, Piece> get pieces => computeGhostPieces();

  /// Computes the ghost pieces map from the current premove queue.
  Map<Square, Piece> computeGhostPieces() {
    final premoveQueue = premoveQueueNotifier.value;
    if (blindfoldMode || premoveQueue.isEmpty) return const {};

    final game = gameNotifier.value;
    final initialPieces = piecesNotifier.value;
    final virtualMap = Map<Square, Piece>.from(initialPieces);
    final movedDestinations = <Square, Piece>{};

    for (final move in premoveQueue) {
      switch (move) {
        case NormalMove(:final from, :final to, :final promotion):
          final p = virtualMap.remove(from);
          if (p != null) {
            final moved = promotion != null ? Piece(color: p.color, role: promotion) : p;
            virtualMap[to] = moved;
            movedDestinations.remove(from);
            movedDestinations[to] = moved;
            if (p.role == Role.king) {
              final rank = from.rank;
              if (from.file == File.e && to.file == File.g) {
                final rook = virtualMap.remove(Square.fromCoords(File.h, rank));
                if (rook != null) {
                  virtualMap[Square.fromCoords(File.f, rank)] = rook;
                  movedDestinations.remove(Square.fromCoords(File.h, rank));
                  movedDestinations[Square.fromCoords(File.f, rank)] = rook;
                }
              } else if (from.file == File.e && to.file == File.c) {
                final rook = virtualMap.remove(Square.fromCoords(File.a, rank));
                if (rook != null) {
                  virtualMap[Square.fromCoords(File.d, rank)] = rook;
                  movedDestinations.remove(Square.fromCoords(File.a, rank));
                  movedDestinations[Square.fromCoords(File.d, rank)] = rook;
                }
              }
            }
          }
        case DropMove(:final role, :final to):
          final side = game?.playerSide == PlayerSide.white ? Side.white : Side.black;
          final p = Piece(color: side, role: role);
          virtualMap[to] = p;
          movedDestinations[to] = p;
      }
    }
    return movedDestinations;
  }

  @override
  void paint(Canvas canvas, Size size) {
    final movedDestinations = computeGhostPieces();
    if (movedDestinations.isEmpty) return;
    ...
    for (final entry in movedDestinations.entries) {
      final square = entry.key;
      final piece = entry.value;
      final asset = pieceAssets[piece.kind];
      if (asset == null) continue;
  ...
  @override
  bool shouldRepaint(GhostPiecesPainter oldDelegate) {
    return imagesLoaded != oldDelegate.imagesLoaded ||
        squareSize != oldDelegate.squareSize ||
        orientation != oldDelegate.orientation ||
        blindfoldMode != oldDelegate.blindfoldMode ||
