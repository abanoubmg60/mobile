...
GhostPiecesPainter? _ghostPiecesPainter(WidgetTester tester) {
  for (final element in find.byType(CustomPaint).evaluate()) {
    final widget = element.widget as CustomPaint;
    if (widget.painter is GhostPiecesPainter) {
      return widget.painter! as GhostPiecesPainter;
    }
  }
  return null;
}

bool _isQueuedPremoveHighlight(WidgetTester tester, Square square) {
  final p = _highlightsPainter(tester);
  return p.interactionNotifier.premoveQueue.any((m) => m.hasSquare(square));
}
...
void main() {
  ...
  group('Multiple premoves', () {
    testWidgets('queue multiple premoves in a chain and render ghost piece at destination', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        const _TestApp(
          fen: 'rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1',
          initialPlayerSide: PlayerSide.white,
          settings: ChessboardSettings(animationDuration: Duration.zero, multiplePremoves: true),
        ),
      );

      // Premove 1: Queen d1 to f3
      await makeMove(tester, Square.d1, Square.f3);
      expect(_isQueuedPremoveHighlight(tester, Square.d1), isTrue);
      expect(_isQueuedPremoveHighlight(tester, Square.f3), isTrue);
      expect(_highlightsPainter(tester).interactionNotifier.premoveQueue.length, 1);

      // Ghost piece at destination f3
      expect(_ghostPiecesPainter(tester)?.pieces[Square.f3], Piece.whiteQueen);

      // Premove 2: Queen chained f3 to f6
      await makeMove(tester, Square.f3, Square.f6);
      expect(_isQueuedPremoveHighlight(tester, Square.f6), isTrue);
      expect(_highlightsPainter(tester).interactionNotifier.premoveQueue.length, 2);

      // Final ghost piece is at f6
      expect(_ghostPiecesPainter(tester)?.pieces[Square.f6], Piece.whiteQueen);
      // f3 intermediate destination is not in ghost pieces anymore
      expect(_ghostPiecesPainter(tester)?.pieces.containsKey(Square.f3), isFalse);
    });

    testWidgets('tapping empty square clears all queued premoves', (WidgetTester tester) async {
      await tester.pumpWidget(
        const _TestApp(
          fen: 'rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1',
          initialPlayerSide: PlayerSide.white,
      ...
      // Tap an empty square (e.g. a3) to cancel all premoves
      await tester.tapAt(squareOffset(tester, Square.a3));
      await tester.pump();

      expect(_highlightsPainter(tester).interactionNotifier.premoveQueue, isEmpty);
      expect(_ghostPiecesPainter(tester)?.pieces.isEmpty, isTrue);
    });

    testWidgets('queue multiple premoves with different pieces', (WidgetTester tester) async {
      await tester.pumpWidget(
        const _TestApp(
          fen: 'rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1',
          initialPlayerSide: PlayerSide.white,
      ...
      // Premove 1: Queen d1 to f3
      await makeMove(tester, Square.d1, Square.f3);
      // Premove 2: Pawn c2 to c4
      await makeMove(tester, Square.c2, Square.c4);

      expect(_highlightsPainter(tester).interactionNotifier.premoveQueue.length, 2);
      expect(_isQueuedPremoveHighlight(tester, Square.d1), isTrue);
      expect(_isQueuedPremoveHighlight(tester, Square.f3), isTrue);
      expect(_isQueuedPremoveHighlight(tester, Square.c2), isTrue);
      expect(_isQueuedPremoveHighlight(tester, Square.c4), isTrue);

      // Both pieces have ghost representations at their targets
      expect(_ghostPiecesPainter(tester)?.pieces[Square.f3], Piece.whiteQueen);
      expect(_ghostPiecesPainter(tester)?.pieces[Square.c4], Piece.whitePawn);
    });
  });
